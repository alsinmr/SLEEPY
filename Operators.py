...
def so_m(S=None):
    ...
    jm=np.zeros(M**2,dtype=dtype)
    jm[M::M+1]=np.sqrt(S*(S+1)-np.arange(-S+1,S+1)*np.arange(-S,S))
    return jm.reshape(M,M).astype(dtype)

def so_p(S=None):
    "Calculates Jp for a single spin"
    if S is None:S=1/2
    M=np.round(2*S+1).astype(int)    
    jp=np.zeros(M**2,dtype=dtype)
    jp[1::M+1]=np.sqrt(S*(S+1)-np.arange(-S+1,S+1)*np.arange(-S,S))
    return jp.reshape(M,M).astype(dtype)
        
def so_x(S=None):
    "Calculates Jx for a single spin"
    return 0.5*(so_m(S)+so_p(S)).astype(dtype)

def so_y(S=None):
    "Calculates Jx for a single spin"
    return 0.5*1j*(so_m(S)-so_p(S)).astype(dtype)

def so_z(S=None):
    "Calculates Jz for a single spin"
    if S is None:S=1/2
    M=np.round(2*S+1).astype(int)
    jz=np.zeros(M**2,dtype=dtype)
    jz[::M+1]=np.arange(S,-S-1,-1)
    return jz.reshape(M,M).astype(dtype)
